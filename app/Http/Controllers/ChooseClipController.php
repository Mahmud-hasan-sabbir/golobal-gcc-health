...
use App\Models\ChooseSlip;

class ChooseClipController extends Controller
{
    public function index(){
        // $nightSlip = NightSlip::get()->toArray();
        return view('admin.Slip.chooseClip.index');
    }
    public function creatChooseSlip(){
        $hospitalName = Hospital::get()->toArray();
        return view('admin.Slip.chooseClip.create',compact('hospitalName'));
    }
    public function storeChooseSlip(Request $request){
        $storeData = new ChooseSlip();
        $storeData->first_name = $request->firstname;
        $storeData->last_name = $request->Lastname;
        $storeData->passport_no = $request->passport_number;
        $storeData->gender = $request->gender;
        $storeData->marital_status = $request->marital_status;
        $storeData->birth_date = $request->birthdate;
        $storeData->passport_issue_date = $request->passport_issue_date;
        $storeData->passport_expiry_date = $request->passport_expiry;
        $storeData->passport_issue_place = $request->passport_issue_place;
        $storeData->traveling_to = $request->country_traveling;
        $storeData->visa_type = $request->visa_type;
        $storeData->traveling_from = $request->country_from;
        $storeData->nationality = $request->nationality;
        $storeData->hospital_ids = $request->job_post;
        $storeData->city_from = $request->city_from;
        $storeData->nid = $request->national_id;
        $storeData->comments = $request->comment;
        $storeData->user_id = Auth()->user()->id;
        $storeData->has_send = 0;
        $storeData->save();
