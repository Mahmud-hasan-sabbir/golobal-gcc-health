<div class="content">
    ...
    <div class="card card-default">
      <table class="table table-striped table-class" id="table-id">
        ...
        <tbody>
          @foreach ($transection as $key => $item)
          <tr>
            ...
            <td>
              @if($item['has_send'] != 1)
              <button type="submit" data-id="{{ $item['id'] }}" onclick="Transaction.edit($(this))"><i class="mdi mdi-circle-edit-outline"></i></button>
              <button type="submit" data-id="{{ $item['id'] }}" onclick="Transaction.send($(this))"><i class="mdi mdi-send"></i></button>
              @else
              Action Done
              @endif
            </td>
  ...
  var Transaction = {
    ...
    send: function(elem){
      $('#loader').removeClass('hidden')
      id = elem.data('id');
      let url = '{{route('send-transaction')}}';
      let data = {id};
      ajaxCallAsyncCallbackAPI(url, data, 'POST', function (response) {
          if (response.status === 'error') {
